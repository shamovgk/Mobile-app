/**
 * Генератор вопросов типа "context" (ИСПРАВЛЕН)
 */
...
export function buildContextSlot(
  ...
  let example: string;
  if (usePlural) {
    example = lex.examplesPlural?.[0] || lex.examples?.[0] || `I like ${pluralForm}.`;
  } else {
  ...
  return {
    ...
    options: [
      { id: singularForm, isCorrect: !usePlural },
      { id: pluralForm, isCorrect: usePlural },
    ].sort(() => rng() - 0.5),
